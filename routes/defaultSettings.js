...
router.put("/:user_id", userShouldBeLoggedIn, async function (req, res, next) {
  ...
  try {
    ...
    const user = results.data[0];
    await db(
      `UPDATE default_settings SET font_size=${font_size}, font_color="${font_color}", background_color="${background_color}", line_spacing=${line_spacing}, fixation=${fixation}, saccade=${saccade} WHERE user_id=${user.id}`
    );
    const { data } = await db(`SELECT * FROM default_settings;`);
    res.status(200).send(data);
  } catch (err) {
